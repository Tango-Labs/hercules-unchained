...
public class FixedFollower : MonoBehaviour
{
    ...
    void Update()
    {
        if (transform.position.x < stopChasingAt)
        {
            ...
            {
                Debug.Log(followWho.transform.position.x - withDistance - transform.position.x);
                Vector3 delta = new Vector3(0-(followWho.transform.position.x - withDistance - transform.position.x), 0, 0);
